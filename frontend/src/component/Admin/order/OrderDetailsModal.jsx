      ...
      <div className="bg-white p-6 rounded-xl w-[90%] md:w-[600px]  max-h-[90vh] overflow-y-auto shadow-lg"
       dir={i18n.language === "fa" || i18n.language === "ps" ? "rtl" : "ltr"}>
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg  font-semibold">{t("table.order_id")} #{order.id}</h2>
          <button onClick={onClose} className="cursor-pointer">
            <X />
          </button>
        </div>

        <p><strong>{t("modal.customer")}:</strong> {order.name}</p>
        <p><strong>{t("modal.phone")}:</strong> {order.phone}</p>
        <p><strong>{t("modal.address")}:</strong> {order.address || "—"}</p>
        <p><strong>{t("modal.order_type")}:</strong> {order.order_type_display}</p>
        <p><strong>{t("modal.status")}:</strong> {order.status_display}</p>
        <p><strong>{t("modal.note")}:</strong> {order.note || "—"}</p>
